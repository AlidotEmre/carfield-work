...
- **R1 — Per-op methods on ratified ground only.** The public surface is one method per existing ioctl — `ping()`, `cluster_run()`, `paging_test()` (debug/test-only), `xform()` (mock-only, §4) — each built on the confirmed contract: an op-specific request struct in, an op-specific status/result out. It is NOT `transfer(data) -> bytes`: the real decrypt flow's output destination is still an open question (see R1 correction below), and even under the older host-writeback model a generic byte-return doesn't fit the multi-buffer shape a real crypto op would need. Echo-shaped APIs are mock-only (R3).
  ...
  **Corrected from v1:** v1 proposed a single `submit(cmd, payload) -> status` primitive with `cmd` as a runtime parameter, reasoning that `letter1 = command` meant command selection was a value passed at the Python/ioctl boundary. It isn't: none of the four existing ioctl request structs (`carfield_ping`, `carfield_cluster_run`, `carfield_paging_test_req`, `carfield_ot_xform_req`) carries a `cmd` field — `CARFIELD_OT_CMD_XFORM` is hardcoded inside `carfield.c`'s `CARFIELD_OT_XFORM` handler (`carfield.c:298`), not read from userspace. The project's own convention, already established across Phase 0-3, is one new ioctl number per new operation (`_IOWR(CARFIELD_MAGIC, N, struct ...)`), not one ioctl multiplexed by a `cmd` value. This is a normal Linux ioctl idiom and not worth fighting. `letter1 = command` remains true, it just happens entirely inside the kernel, one layer below where Python ever sees it — the driver already picks the right `letter1` for whichever ioctl was called. R1's actual goal (don't build an echo-shaped generic API) survives unchanged; only the shape of the *plumbing* changes, from "one primitive + cmd arg" to "shared internal helper + one public method per op".
...
- **Error mapping is per-op, not one global table**, because the same errno means different things depending on which ioctl produced it — see the collisions below. Every raised exception is `CarfieldError(op: str, errno: int, message: str)`, so the exception itself always says which ioctl it came from.
  ...
  **Corrected from v1:** v1's mapping table (`ETIMEDOUT→CarfieldTimeout`, `EILSEQ→CarfieldBadHeader`, `EINVAL→CarfieldBadRequest`, `ERANGE→CarfieldAddressRange`, `E2BIG→CarfieldSizeError`) was incomplete and, worse, two of its errnos are reused by the driver for unrelated failures on the *same* ioctl:
  - `-EFAULT` is both `CARFIELD_MOCK_OT_ERR_MAP`'s mapped errno (`carfield_mock_ot.c:132`) **and** what `copy_from_user`/`copy_to_user` return on a bad pointer (`carfield.c:290-291,306,317`) — inside the *same* `CARFIELD_OT_XFORM` call. Per-op scoping alone doesn't separate these two, because they're the same op. Fix: `device.py` pre-fills `ot_status` with `CARFIELD_OT_STATUS_NONE` (`0xFFFFFFFF`, already defined in `carfield_mock_ot.h:34` for exactly this "no reply happened" purpose) before the ioctl call. `fcntl.ioctl()` mutates the passed buffer in place whenever `copy_to_user` succeeded, regardless of the ioctl's return value — so on catching `EFAULT`, checking whether `ot_status` is still the sentinel or was overwritten with a real status distinguishes "copy fault, request never even reached the mock" from "mock rejected on `ERR_MAP`".
  ...
  - Missing from v1 entirely: `EBADMSG` (`ERR_GEOMETRY`), `EIO` (`mock_force_err` / unrecognized status, `carfield_mock_ot.c:134`), `ENODEV` (`CARFIELD_OT_XFORM` when `mock_ot=0`, `carfield.c:288`). All added to the per-op table below.
...
- Built on `CarfieldDevice._paging_op(CARFIELD_OT_XFORM, CarfieldOtXformReq, addr, size)`.
...
**Post-implementation addendum (2026-07-31):** `CARFIELD_MOCK_OT_XFORM`/`carfield_mock_ot_req` (and the shared cmd/sentinel constants `CARFIELD_MOCK_OT_CMD_XFORM`, `CARFIELD_MOCK_OT_CMD_CLUSTER_BOOT`, `CARFIELD_MOCK_OT_STATUS_NONE`, and the `mock_status` field) were renamed to `CARFIELD_OT_XFORM`/`carfield_ot_xform_req`/`CARFIELD_OT_CMD_XFORM`/`CARFIELD_OT_CMD_CLUSTER_BOOT`/`CARFIELD_OT_STATUS_NONE`/`ot_status` — these are shared with the real `real_mbox=1` backend (`carfield_mbox_hw.c`) and always were, so "mock" in their names was misleading (flagged as a known smell in `carfield.c`'s own comment since the hw backend was added). The mock kthread's own implementation (`carfield_mock_ot.c`'s functions, the `CARFIELD_MOCK_OT_OK`/`ERR_*` status codes, the `mock_*` module params) correctly keeps "mock" in its name and was not touched. `abi.py`/`device.py`/`demo.py` updated accordingly (`CarfieldMockOtReq` → `CarfieldOtXformReq`).
