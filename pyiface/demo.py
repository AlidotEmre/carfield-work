...
def xform(dev, data: bytes) -> bytes:
    """MOCK-ONLY. Sends `data` through ALSAQR_OT_XFORM (requires the
    ...
    AlsaqrError subclass on any §5 rejection, ENODEV if neither backend
    is enabled, or AlsaqrTimeout on mock_no_reply.
    """
    size = len(data)
